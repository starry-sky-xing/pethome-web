...
<script>
    export default {
        ...
        methods:{
            ...
            //查找多级部门
            loadonetwoDept(){
               var options =   this.$http.post("/dept/getDepartments").then((res)=>{
                    //console.debug(res);
                    //给options赋值
            ...
            handleEdit(index,row){
                ...
                this.loademployees();
                //查找多级部门
                let myoptions = this.loadonetwoDept();
                //console.debug(myoptions);
                //数据回显
                for(var v in this.addForm){
                ...
                if(row.dirPath){
                    let dirPath=row.dirPath.split(",");
                    //上级部门是一个数组
                    this.addForm.prients=dirPath;
                    //console.debug(dirPath);
                }
                //禁用自己选项
                let myPath = row.dirPath.split(",");//拿到自己的部门
                myPath.push(row.id.toString());
                //console.debug(myPath);
                //封装对象
                // let disable={
                //     myoptions:myoptions,
                //     myPath:myPath,
            ...
            getOptions(val){
                // console.debug(val);
                //console.debug(val.myoptions);
               for(var k in val){
                   console.debug(val);

               }
                // val.myoptions.forEach(item => {
                //     if (item.value == val.myPath[val.myIndex]) { //如果id是第一级部门
                //         if(item==val.myPath.length-1){  //找到了自己部门层级
                //             console.debug(item);
                //             item.disabled = true;  //添加禁用
                //         }else {
                //             let disable={
                //                 myoptions:item.children,
                //                 myPath:val.myPath,
                //                 myIndex:val.myIndex+1
                //             };
                //             this.getOptions(disable); //如果没有找到自己层级就调用递归 索引加一
                //         }
                //     }
                // });
                // return val ;
            ...
            batchRemove(){
                //console.debug(this.sels);
                this.$http.post("/dept/batchDelete",this.sels).then((res)=>{
                    //console.debug(res)
                    let {success,msg} = res.data;
                    if(success){
                        this.$message({
